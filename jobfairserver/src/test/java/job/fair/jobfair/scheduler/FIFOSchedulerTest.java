...
public class FIFOSchedulerTest extends TestCase {
    ...
    public void testRun() throws InterruptedException {
        ...
        // when the job1 got applied
        Scheduler s = schedulerSvc.createOrGetFIFOScheduler("job1");
        assertEquals(0, s.getJobsRunning().size());
        assertEquals(0, s.getJobsWaiting().size());

        // add the candidate into the Scheduler
        Candidate c1 = new Candidate();
        c1.setId(1L);
        s.submit(c1);
        assertEquals(1, s.getJobsWaiting().size());

        //	s.removeFromWaitingQueue(1L);
        //	assertEquals(0, s.getJobsWaiting().size());
//
//		s.submit(c1);
//		s.submit(c2);
//		Thread.sleep(200);
//
//		assertEquals(2, s.getJobsWaiting().size());
...
//		Scheduler s = schedulerSvc.createOrGetFIFOScheduler("Preload51job");
//		assertEquals(0, s.getJobsRunning().size());
//		assertEquals(0, s.getJobsWaiting().size());
//
